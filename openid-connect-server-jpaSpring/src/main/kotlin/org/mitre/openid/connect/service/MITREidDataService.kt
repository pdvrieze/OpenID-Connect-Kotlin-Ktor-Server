...
*/
    ...
    fun importAuthenticationHolder(context: DataServiceContext, ahe: AuthenticationHolder) {
        with(context) {
            val currentId: Long = requireNotNull(ahe.id) { "Missing id for authentication holder" }
            val holderWithoutId = ahe.copy(null)

            val newId = authHolderRepository.save(holderWithoutId).id!!
    ...
    fun fixObjectReferences(context: DataServiceContext) {
        with(context) {
            for ((oldRefreshTokenId, clientRef) in maps.refreshTokenToClientRefs) {
                ...
                refreshToken.client = client?.let(ClientDetailsEntity.Companion::from)
            ...
            for ((oldAccessTokenId, clientRef) in maps.accessTokenToClientRefs) {
                ...
                accessToken.client = client?.let(ClientDetailsEntity.Companion::from)
...
//                refreshToken?.let { accessToken.refreshToken = it }
    ...
    interface ConfigurationData {
        ...
        val authenticationHolders: List<AuthenticationHolder>
    ...
    open class ConfigurationData10 : ConfigurationDataBase {
        ...
        override val authenticationHolders: List<AuthenticationHolder>
    ...
    companion object {
        ...
        private val DUMMY_AUTH_HOLDER = AuthenticationHolder.DUMMY
    }
}
