package org.mitre.openid.connect.ktor.assertion
...
class TestKtorJWTBearerAuthenticationProvider {
    @Mock
...
//        assertFalse(jwtBearerAuthenticationProvider.supports(UsernamePasswordAuthenticationToken::class.java))
    ...
    fun should_throw_AuthenticationServiceException_for_SignedJWT_when_unsupported_authentication_method_for_SignedJWT() {
        val unsupportedAuthMethods =
            Arrays.asList(null, OAuthClientDetails.AuthMethod.NONE, OAuthClientDetails.AuthMethod.SECRET_BASIC, OAuthClientDetails.AuthMethod.SECRET_POST)
    ...
    fun should_throw_AuthenticationServiceException_for_SignedJWT_when_invalid_algorithm_for_PRIVATE_KEY_auth_method() {
        ...
        for (algorithm in invalidAlgorithms) {
            ...
            whenever(token.jwt).thenReturn(signedJWT)
            whenever(client.tokenEndpointAuthMethod).thenReturn(OAuthClientDetails.AuthMethod.PRIVATE_KEY)
    ...
    fun should_throw_AuthenticationServiceException_for_SignedJWT_when_invalid_algorithm_for_SECRET_JWT_auth_method() {
        ...
        for (algorithm in invalidAlgorithms) {
            val signedJWT = createSignedJWT(algorithm)
            whenever(token.jwt).thenReturn(signedJWT)
            whenever(client.tokenEndpointAuthMethod).thenReturn(OAuthClientDetails.AuthMethod.SECRET_JWT)
            whenever(client.tokenEndpointAuthSigningAlg).thenReturn(algorithm)
    ...
    fun should_return_valid_token_when_audience_contains_token_endpoint(): Unit = runBlocking {
        ...
        Assertions.assertTrue(token.authorities.map { LocalGrantedAuthority(it.authority) }
                                  .containsAll(listOf(authority1, authority2, authority3)))
        Assertions.assertEquals(4, token.authorities.size)
    }

    @Test
    fun should_return_valid_token_when_issuer_does_not_end_with_slash_and_audience_contains_token_endpoint(): Unit =
        runBlocking {
            val jwtClaimsSet = JWTClaimsSet.Builder()
                .issuer(CLIENT_ID)
                .subject(SUBJECT)
