...
var ListWidgetChildView = Backbone.View.extend({
    ...
    events: {
        "click .btn-delete-list-item": 'deleteItem', "change .checkbox-list-item": 'toggleCheckbox'
    },
    ...
    deleteItem: function (e) {
        ...
        this.model.destroy({
            dataType: false, processData: false, error: app.errorHandlerView.handleError()
        });
    ...
    render: function () {
        ...
        var data = {
            model: this.model.toJSON(), opt: this.options
        };
...
var ListWidgetView = Backbone.View.extend({
    ...
    events: {
        "click .btn-add-list-item": "addItem", "keypress": function (e) {
            // trap the enter key
            if (e.which == 13) {
                e.preventDefault();
                this.addItem(e);
    ...
    render: function (eventName) {
        ...
        this.$el.html(this.template({
            placeholder: this.options.placeholder, helpBlockText: this.options.helpBlockText
        }));
        ...
        if (_.size(this.collection.models) == 0 && _.size(this.options.autocomplete) == 0) {
            ...
            if (this.options.autocomplete) {
                _.each(this.options.autocomplete, function (option) {
                    ...
                    var el = new ListWidgetChildView({
                        model: model, toggle: true, checked: checked, collection: _self.collection
                    }).render().el;
            ...
            _.each(values.models, function (model) {
                var el = new ListWidgetChildView({
                    model: model, collection: _self.collection
                }).render().el;
                $("tbody", _self.el).append(el);
            }, this);
        }
            ...
            if (key && value) {
                ...
                if (typeof (value) === 'object') {
                    ...
                    _.each(value, function (value, key) {
                        $('dl', el).append(t({
                            key: key, value: value, category: k
                        }));
                    });
    ...
    handleError: function (message) {
        ...
        return function (model, response, options) {
            ...
            _self.showErrorMessage(_self.headerTemplate({
                message: message, model: model, response: response, options: options
            }), _self.template({
                message: message, model: model, response: response, options: options
            }));
    ...
    $.when.apply(null, ui.templates.map(loader)).done(function () {
        ...
        $.getJSON(base + '.well-known/openid-configuration', function (data) {
            ...
            Backbone.history.start({
                pushState: true, root: baseUrl.attr('relative') + 'manage/'
            });
        });
    });
