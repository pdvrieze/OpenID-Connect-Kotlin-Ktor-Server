...
//        super.importData(configJson)
    ...
    protected fun importData(conf: ExtendedConfiguration) {
        val context = Context(clientRepository, approvedSiteRepository, wlSiteRepository, blSiteRepository, authHolderRepository, tokenRepository, sysScopeRepository, extensions, maps)
...
*/
    ...
    @Throws(IOException::class)
    override fun Context.readClients(config: List<ClientDetailsConfiguration>) {
    ...
    @Throws(IOException::class)
    override fun Context.readGrants(config: List<ApprovedSite.SerialDelegate>) {
    ...
    @Throws(IOException::class)
    override fun Context.readWhitelistedSites(sites: List<WhitelistedSite>) {
        for (wlSite in sites) {
    ...
    @Throws(IOException::class)
    override fun Context.readBlacklistedSites(sites: List<BlacklistedSite>) {
        for (blSite in sites) {
    ...
    @Throws(IOException::class)
    override fun Context.readAuthenticationHolders(holders: List<AuthenticationHolderEntity>) {
        for (ahe in holders) {
    ...
    override fun Context.readAccessTokens(config: List<OAuth2AccessTokenEntity.SerialDelegate>) {
        for (delegate in config) {
            val currentId: Long = delegate.currentId
    ...
    override fun Context.readRefreshTokens(tokenDelegates: List<OAuth2RefreshTokenEntity.SerialDelegate>) {
     ...
     */
    @Throws(IOException::class)
    override fun Context.readSystemScopes(scopes: List<SystemScope>) {
        for (scope in scopes) {
            sysScopeRepository.save(scope)
        }
